// Export functionality for Roy's Ploy (CommonJS/IIFE pattern)
// Depends on: Utils, Storage
(function(global) {
    'use strict';
    ...
    function getExportFilename(prefix, extension) {
        ...
        const shortId = global.Utils.generateShortId();
    ...
    async function exportJSON(state) {
        const json = global.Storage.exportDocument(state);
    ...
    async function shareJSON(state) {
        if (!global.Utils.supportsWebShare()) {
    ...
    // CSV Exports
    async function exportProjectsCSV(state) {
        ...
        const rows = state.projects.map(p => [
            ...
            global.Utils.formatDateShort(p.createdAt),
            global.Utils.formatDateShort(p.updatedAt),
            p.cadence?.type || 'weekly',
            p.lastReviewAt ? global.Utils.formatDateShort(p.lastReviewAt) : 'Never'
        ]);
    ...
    async function exportLogsCSV(state) {
        ...
        const rows = state.logs.map(l => {
            ...
            return [
                ...
                l.nextExcited || '',
                l.impactNote || '',
                global.Utils.formatDateShort(l.createdAt)
            ];
        });
    ...
    async function exportWinsCSV(state) {
        ...
        const rows = state.wins.map(w => {
            ...
            return [
                ...
                w.kind,
                w.note,
                global.Utils.formatDateShort(w.createdAt)
            ];
        });
    ...
    async function exportIdeasCSV(state) {
        ...
        const rows = state.ideas.map(i => {
            ...
            return [
                ...
                i.text,
                i.archivedAt ? global.Utils.formatDateShort(i.archivedAt) : '',
                i.promotedToProjectId || '',
                global.Utils.formatDateShort(i.createdAt)
            ];
        });
    ...
    function generateCSV(headers, rows) {
        const headerRow = headers.map(global.Utils.escapeCsv).join(',');
        const dataRows = rows.map(row => row.map(global.Utils.escapeCsv).join(',')).join('\n');
        return `${headerRow}\n${dataRows}`;
    }
    
    // ICS (iCalendar) Export
    async function exportProjectICS(project) {
        const ics = generateICS([project]);
        const slug = global.Utils.slugify(project.title);
        const filename = `review-${slug}.ics`;
        downloadFile(ics, filename, 'text/calendar;charset=utf-8');
        return filename;
    }
    
    async function exportAllActiveReviewsICS(state) {
    ...
    function generateICS(projects) {
        ...
        const events = projects.map(project => {
            const nextReview = global.Utils.getNextReviewDate(project.cadence);
